...
const TimelineItem: React.FC<{ iconName: string; year: string; title: string; children: React.ReactNode; isLast?: boolean }> = ({ iconName, year, title, children, isLast = false }) => (
    <div className="relative pl-12">
        ...
        <div className="flex items-baseline gap-4">
            <span className="bg-techflex-blue-600 text-white text-sm font-semibold px-3 py-1 rounded-full">{year}</span>
...
const TeamMemberCard: React.FC<{ imageUrl: string; name: string; title: string; }> = ({ imageUrl, name, title }) => (
    <div className="text-center">
        <div className="relative w-40 h-40 mx-auto">
            <img src={imageUrl} alt={name} className="rounded-full w-full h-full object-cover shadow-xl" />
        </div>
        <h3 className="mt-6 text-xl font-bold text-brand-gray-900">{name}</h3>
        <p className="text-techflex-blue-600 font-semibold">{title}</p>
    </div>
);
        ...
        <div className="bg-brand-gray-50">
            ...
            <section className="bg-brand-gray-100 py-20 sm:py-28">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 justify-center max-w-4xl mx-auto">
                        {team.map((member) => (
            ...
            <section className="bg-techflex-blue">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-20">
                    <div className="text-center text-white">
                        ...
                        <div className="mt-8">
                             <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('careers'); }} className="bg-techflex-orange hover:bg-techflex-orange-600 text-white font-bold py-4 px-10 rounded-lg text-lg transition-all duration-300 transform hover:scale-105">
                                View Careers
                            </a>
                        </div>
                    </div>
