                ...
                <div>
                    ...
                    <div className="mt-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-brand-gray-500">{new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div onClick={handleCardClick} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 group overflow-hidden flex flex-col cursor-pointer">
            ...
            <div className="p-6 flex-grow flex flex-col">
                ...
                <div className="mt-4 pt-4 border-t border-brand-gray-100 flex items-center gap-3">
                    <img src={post.authorAvatarUrl} alt={post.author} className="w-10 h-10 rounded-full" />
                    <div>
                        <p className="text-sm font-semibold text-brand-gray-800">{post.author}</p>
                        <p className="text-xs text-brand-gray-500">{new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                    </div>
                </div>
            </div>
        </div>
